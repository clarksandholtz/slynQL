...
const messages = {
  async createMessage (parent, args, ctx, info) {
    ...
    publish(userId, Types.newMessage, message)
  ...
  async createMessages (parent, args, ctx, info) {
    ...
    publish(userId, Types.syncComplete, {success: true})
    return {
      success: true,
  ...
  async sendMessage (parent, args, ctx, info) {
    ...
    publish(userId, Types.pendingMessages, pendingMsg)
    return pendingMsg
  },

  async updateMessage (parent, args, ctx, info) {
    getUserId(ctx) // authorize
    const result = await ctx.db.mutation.updateMessage({
      data: {
        read: args.read,
        error: args.error
      },
  ...
  async deleteAllMessages (parent, args, ctx, info) {
    ...
    for (let x = 0; x < conversations.length; x++) {
      ...
      for (let y = 0; y < files.length; y++) {
        if (!files[y].deleted) {
          let file = path.join(__dirname, '/../../../Images/', files[y].content)
